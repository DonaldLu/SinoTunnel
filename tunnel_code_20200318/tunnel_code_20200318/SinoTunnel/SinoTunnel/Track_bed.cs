        ...
        public void Execute(UIApplication app)
        {
            ...
            try
            {
                ...
                foreach (Tuple<IList<data_object>, setting_station> all_list in list)
                {
                    ...
                    FailureHandlingOptions failureHandlingOptions = big_t.GetFailureHandlingOptions();
                    FailureHandler failureHandler = new FailureHandler();
                    failureHandlingOptions.SetFailuresPreprocessor(failureHandler);
                    failureHandlingOptions.SetClearAfterRollback(true);
                    big_t.SetFailureHandlingOptions(failureHandlingOptions);
                    ...
                    for (int i = 1; i < data_list.Count; i++)
                    {
                        ...
                        for (int k = 0; k < setting_Station.third_rail_station.Count; k++)
                        {
                            ...
                            if (current_station >= start_st && current_station < end_st)
                            {
